...
IncompressibleSPH::IncompressibleSPH()
...
void IncompressibleSPH::setup(float containerWidth, float containerHeight)
{
	m_containerWidth = containerWidth;
	m_containerHeight = containerHeight;

	m_params = ci::params::InterfaceGl::create(ci::app::getWindow(), "IncompressibleSPH", ci::app::toPixels( ci::Vec2i( 300, 400)));
...
void IncompressibleSPH::update(const float deltaTime)
{
...
void IncompressibleSPH::mouseDown(const ci::app::MouseEvent& event)
{
}

void IncompressibleSPH::drawParams()
{
	m_params->draw();
}

void IncompressibleSPH::reset()
{
	initParticles();
}

void IncompressibleSPH::initParticles()
{
...
void IncompressibleSPH::initBoundaries()
{
...
void IncompressibleSPH::updateParticleDensity()
{
	for(unsigned int i = 0; i < m_particles.size(); i++)
	{
		...
		float k = 1.0f;
		float y = 7.0f;
		float pressure0 = (k * m_restDensity) / y;
		float pressure1 = std::pow((density/m_restDensity), y) - 1.0f;
		float pressure = pressure0 * pressure1;

		//m_particles[i].setPressure(density - m_restDensity);
		m_particles[i].setPressure(pressure);

		// LOG_INFO_STREAM(<< "particle " << i << ": density - " << density << " | pressure - " << density - m_restDensity);
	}
}

void IncompressibleSPH::calculatePressureForces(const float deltaTime)
{
	for(unsigned int i = 0; i < m_particles.size(); i++)
	{
		Vec2f pos = m_particles[i].getPosition();
...
void IncompressibleSPH::calculateViscosity(const float deltaTime)
{
	for(unsigned int i = 0; i < m_particles.size(); i++)
	{
		Vec2f pos = m_particles[i].getPosition();
...
void IncompressibleSPH::calculateBoundaryForces(const float deltaTime)
{
	for(unsigned int i = 0; i < m_particles.size(); i++)
	{
		for(unsigned int j = 0; j < m_boundaries.size(); j++)
		{
			if(m_boundaries[j].checkHit(m_particles[i].getPosition()))
			{
				...
				if(forceMagnitude != 0.0f)
				{
					...
					distance = std::abs(distance * distance * distance) * (distance / std::abs(distance));
				}
				else
				{
					distance = 99999.9f;
...
float IncompressibleSPH::distanceKernel(const float distance)
{
	if(distance > m_h)
	{
		return 0.0f;
...
Vec2f IncompressibleSPH::distanceKernelGradient(const Vec2f& pos0, const Vec2f& pos1)
{
...
float IncompressibleSPH::distanceKernelViscosity(const float distance)
{
	if(distance > m_h)
	{
		return 0.0f;
